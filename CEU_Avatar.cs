...
namespace Gadget
{
    ...
    public class CEU_Avatar : MonoBehaviour
    {
        ...
        public Vector3 rootRotationOffset = new Vector3(90f, 0f, 0f); // リーダーの設定値(90)
        ...
        public AxisMap rootRotY = AxisMap.InvertZ; // リーダーの正解 (rz * -1f)
        public AxisMap rootRotZ = AxisMap.Y;       // リーダーの正解 (ry)
                ...
                using (BinaryReader br = new BinaryReader(ms))
                {
                    ...
                    for (int i = 0; i < numBones; i++)
                    {
                        ...
                        if (hasPos)
                        {
                            ...
                            // 🌟 Root(255)の時はインスペクターの設定（AxisMap）に従ってSwizzleを生成
                            if (b_id == 255) 
                            {
