    ...
    public class CEU_Avatar_v1 : MonoBehaviour
    {
        ...
        public string GenerateRequestBonesJson()
        {
            ...
            if (rigType == RigType.Humanoid)
            {
                ...
                {
                    Debug.LogWarning($"<color=#FF9900>[CEU Avatar]</color> Humanoid Avatar is not set on the Animator for {gameObject.name}. Please set RigType to Generic or assign an Avatar.");
                }
            }
            else 
            {
